...
- Adopt 5-Point Landmark Alignment: Integrate 5-point landmark detection (eyes, nose, mouth corners) with a `CGAffineTransform` (Similarity Transform) to warp the face into a canonical, upright frontal view _before_ feeding it to MobileNetV3 for embedding. This is critical for high accuracy.
