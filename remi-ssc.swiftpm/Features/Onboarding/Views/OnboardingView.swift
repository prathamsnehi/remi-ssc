            ...
            ZStack {
                ...
                VStack {
                    ...
                    VStack {
                        ...
                        // Calculate initial large logo size based on screen dimensions
                        // Base size 120 (iPhone), scale up for iPad (approx 30% of min dimension)
                        // This ensures it looks good on both iPhone (393*0.3 ~= 118) and iPad (768*0.3 ~= 230)
                        let minDimension = min(geometry.size.width, geometry.size.height)
                        let initialLogoSize = max(120, minDimension * 0.3)
                        
                        HStack(spacing: path.isEmpty ? 16 : 8) {
                            Image("Logo")
                                .resizable()
                                .scaledToFit()
                                // Step 0: initialLogoSize (Responsive), Step 1+: 80 (Hero Normal), Step B: 24
                                .frame(width: path.isEmpty ? (introStep == 0 ? initialLogoSize : 80) : 24, 
